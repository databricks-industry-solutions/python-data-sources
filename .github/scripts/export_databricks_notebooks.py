...
COLORS = {"primary": "#FF3621", "text": "#1B3139", "bg": "#FFFFFF", "light_bg": "#F5F5F5", "border": "#E3E3E3"}
...
def list_workspace_notebooks():
    ...
    response = requests.get(
        f"{DATABRICKS_HOST}/api/2.0/workspace/list", headers=headers, params={"path": workspace_path}
    )
...
def export_notebook_html(notebook_path):
    ...
    response = requests.get(
        f"{DATABRICKS_HOST}/api/2.0/workspace/export", headers=headers, params={"path": notebook_path, "format": "HTML"}
    )
